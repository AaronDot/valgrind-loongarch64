                             ...
                             UInt src1, UInt src2, UInt src3 )
{
   ...
   IRExpr* call = mkIRExprCCall(Ity_I64, 0/*regparms*/,
                                ...
                                &loongarch64_calculate_VFCSR,
                                arg);
   putFCSR(2, unop(Iop_64to32, call));
}
   ...
   switch (insTy) {
      case 0b001: {
         ...
         for (i = 0; i < 4; i++) {
            ...
            k32[i] = newTemp(Ity_I32);
            a32[i] = newTemp(Ity_I32);
            assign(j32[i], binop(Iop_GetElem32x4, getVReg(vj), mkU8(i)));
            assign(k32[i], binop(Iop_GetElem32x4, getVReg(vk), mkU8(i)));
            assign(a32[i], binop(Iop_GetElem32x4, getVReg(va), mkU8(i)));
         }

         for (i = 0; i < 4; i++) {
            tmp[i] = newTemp(Ity_I32);
            assign(tmp[i], unop(Iop_ReinterpF32asI32,
                                qop(Iop_MAddF32, rm,
                                    unop(Iop_ReinterpI32asF32, EX(j32[i])),
                                    unop(Iop_ReinterpI32asF32, EX(k32[i])),
                                    unop(Iop_ReinterpI32asF32, EX(a32[i])))));
         }
